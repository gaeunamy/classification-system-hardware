...
- 사과 감지 시 이미지를 캡처하여 서버(Rpi)의 로컬 폴더 내에 저장, 폴더를 1초마다 확인하고 새로운 JPG 파일을 감지하여 <br>Firebase Storage로 업로드
  - 정상 사과는 normal_apple_YYYY-MM-DD_HH:MM:SS.jpg, 불량 사과는 rotten_apple_YYYY-MM-DD_HH:MM:SS.jpg로 자동 저장
...
### 분류 시스템 
- ## 서버-클라이언트 통신
  ① 웹캠에서 실시간으로 640x480 해상도의 프레임을 캡처하고, 이를 JPG 형식으로 인코딩하여 소켓 통신을 통해 클라이언트(PC)로 전송 <br>
  ② 클라이언트는 수신한 프레임을 YOLOv5 모델에 입력으로 제공하여 사과의 품질을 판별 <br>
  ③ 판별 결과(정상/불량)와 신뢰도 점수는 다시 서버로 전송되며, 서버는 이 결과에 따라 서보모터를 제어하여 사과를 분류 
